...
class ProfileReader(object):
	...
	def getSettingsToSlice(cls, printer, nozzle, filament, quality, overrides):
		...
		profile_path = slicingManager.get_slicer_profile_path("cura2")+'/'
		...
		for field in overrides:
			if field == "fill_density":
				...
				if settings['infill_pattern'] == 'grid':
					multiplier = 2
				elif settings['infill_pattern'] in ['triangles','cubic','cubicsubdiv']:
					multiplier = 3
				elif settings['infill_pattern'] in ['tetrahedral','quarter_cubic']:
					multiplier = 2
				elif settings['infill_pattern'] in ['cross','cross_3d']:
					multiplier = 1
